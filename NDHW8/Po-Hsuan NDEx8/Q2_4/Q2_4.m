%% Neural Dynamics Ex8
%% Q2.1 Amari type model with non-linear threshold
...
funW = @(x) A*(abs(x)<a)-B*(abs(x)<=b && abs(x)>=a);
funS = @(x)  C*(1-abs(x)/d)*(abs(x)<d);
H = 1;           % Global inhibition


dt = 0.1e-1 ; % time step size
T_t = 1000e-1;     % total modeling time 
...
%% initial condition 1
%U= -H*ones(num,1);   % initially, all neurons has 0 potential in the first timestep.
...
for j = 1:num
        ...
        U(j,1)= 3.3*C*(1-abs(a_po)/d)*(abs(a_po)<d)-H;
end
...
for step = 2: length(Num_t)  % time step iteration
    ...
    Int = Int + 0.1*funW(b_po-a_po)*(U(i,step-1)>0)*h; 
    end
        
    S =0; funS(a_po); %+ sd*randn(1,1);
    
    U(j,step) = U(j,step-1) + dt*(-U(j,step-1) +Int + S-H)/tau;  % Backward-Euler.
